...
pub async fn translate_code( source_code: &str,target_language: &str,source_language: &str) -> Result<String, Box<dyn std::error::Error>> { // Return type changed to Result<String, ...> to return the translation
//Translate Code
    let api_key = env::var("GPT3_API_KEY").expect("GPT3_API_KEY must be set");
    let client = Client::new();
    let mut headers = HeaderMap::new();
